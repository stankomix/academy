  ...
  .done(function(answer){
     if ( answer.error ){
       ...
       if (answer.error === 'MAX_WORK_ORDER_LIMIT' || answer.error === 'EIGHT_HOURS') {
          ...
          $('#bbform').attr('onsubmit', 'submitOvertimeRequest()');
          ...
          if (answer.error === 'MAX_WORK_ORDER_LIMIT') {
            notify('The Work Order limit is 2 timecards per day. Please submit an overtime request below if you wish to continue');
          }

          if (answer.error === 'EIGHT_HOURS') {
            notify('You have clocked over 8 hours today. Please submit an overtime request below if you wish to continue');
          }
...
function submitOvertimeRequest()
{
  ...
  $.ajax({
    type: 'POST',
    url :'/timecard/create_overtime_request',
    data: $('form#bbform').serialize(),
  })
  .done(function(answer){
     //Close the form
