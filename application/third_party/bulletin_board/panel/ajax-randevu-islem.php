...
$testinfo=mysql_fetch_array(mysql_query("SELECT * FROM test_offline WHERE id='$test_id' order by id asc"));

$score=htmlspecialchars(hcevir($_REQUEST[score]),ENT_QUOTES);
$status=htmlspecialchars(hcevir($_REQUEST[status]),ENT_QUOTES);

$bbeditsql = "UPDATE test_offline SET score='$score',status='$status' WHERE id='$testinfo[id]'";
