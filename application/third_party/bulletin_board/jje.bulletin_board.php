...
$galleryinfo=mysql_fetch_array(mysql_query("SELECT * FROM bb_photos WHERE bb_id='$bb[id]' order by id asc LIMIT 0,1"));
?>
<div class="bbtek" id="bbtek<? echo "$bb[id]"; ?>">
<? if($galleryinfo[id]!=""){?><div class="rsm"><img src="../<? echo "$galleryinfo[small_url]"; ?>" /></div><?}?>
<div class="bbicnlr">
<a href="javascript:;" onclick="bbedit('<? echo "$bb[id]"; ?>');" ><div class="fl eicn <? if($galleryinfo[id]!=""){ echo "icnrk";}?>"><img src="images/edit.png" /></div></a><div class="fl icn <? if($galleryinfo[id]!=""){ echo "icnr";}?>"><img src="images/bbico<? echo "$bb[category]"; ?>.png" class="ico<? echo "$bb[category]"; ?>"/></div><a href="javascript:;" onclick="bbdelete1('<? echo "$bb[id]"; ?>');" id="bdelete<? echo "$bb[id]"; ?>"><div class="fl dicn <? if($galleryinfo[id]!=""){ echo "icnrk";}?>" id="dicn<? echo "$bb[id]"; ?>"><img src="images/delete.png" /></div></a><div class="t"></div>
</div>
