...
$galleryinfo=mysql_fetch_array(mysql_query("SELECT * FROM files WHERE id='$idne' and status='1' order by id asc"));
if($galleryinfo[id]==""){header("Location:filemanager.php");exit();}
$manyfiles=mysql_fetch_array(mysql_query("SELECT COUNT(id) as kd FROM photos where file_id='$galleryinfo[id]' and status='1' order by status asc"));
...
<div class="dtybslk fmbslk"><a href="filemanager.php">File Manager</a><img src="images/sagok.png" /><a href="fm_cat.php?id=4">Event Photos</a><img src="images/sagok.png" /><span><? echo "$galleryinfo[title]"; ?></span><div class="t"></div></div>
<div class="dtybbslk"><? echo "$galleryinfo[title]"; ?> (<? echo "$manyfiles[kd]"; ?>)</div>


<div class="gltum">
<?
$bb_data = "select * from photos where file_id='$galleryinfo[id]' and status='1' order by id desc";
